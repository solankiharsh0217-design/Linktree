...
export default function CloudButton({ link }: Props) {
  return (
    ...
    >
      {/* Thumbnail or Default Logo */}
      {link.thumbnail_url ? (
        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl overflow-hidden flex-shrink-0 bg-indigo-100/60 border border-indigo-200/50 shadow-sm group-hover:border-white/30 group-hover:bg-white/10 transition-colors">
          <Image
        ...
        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl sm:rounded-2xl overflow-hidden flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
          <DefaultLogoIcon icon={link.icon} size="md" />
        </div>
      ) : null}
